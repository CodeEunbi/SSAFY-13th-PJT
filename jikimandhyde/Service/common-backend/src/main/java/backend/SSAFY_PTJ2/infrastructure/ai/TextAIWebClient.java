    ...
    private AnalysisResult convertToAnalysisResult(TextAIResponse response, TextAIRequest request) {
        ...
        var textResults = response.getFilteredElements().stream()
                .map(filteredElement -> {
                    ...
                    // FilteredText들을 TextRange로 변환
                    var hatefulRanges = filteredElement.getFilteredTexts().stream()
                            .map(filteredText -> {
                                // detectedLabels에서 첫 번째를 카테고리로 사용
                                String category = filteredText.getDetectedLabels().isEmpty() ?
                                    "CLEAN" : filteredText.getDetectedLabels().get(0);

                                // confidence에서 가장 높은 점수 찾기
